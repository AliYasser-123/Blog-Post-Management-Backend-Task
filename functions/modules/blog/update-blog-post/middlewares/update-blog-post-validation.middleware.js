...
const { getPromiseConnection, endPromiseConnection } = require('../../../dependencies/config/config');
const { verifyToken } = require('../../../dependencies/global-middlewares/verify-token');
const { attachGlobalMiddlewares } = require('../../../dependencies/utils/add-middlewares');
const { updateBlogPostValidation } = require('./middlewares/update-blog-post-validation');
...
firebaseFunction.put('/blog/:id', verifyToken, updateBlogPostValidation, async (req, res) => {
    try {
        ...
        if (!title && !content && !tags) {
            return res.status(400).send({ message: 'No fields to update', data: null });
        }
        ...
        if (result.affectedRows === 0) {
            return res.status(404).send({ message: 'Blog post not found', data: null });
        }

        // Return success response
        return res.status(200).send({ message: 'Blog post updated successfully', data: null });
    } catch (err) {
        console.error('Error updating blog post:', err);

        // Handle errors and close the connection
        await endPromiseConnection(getPromiseConnection());
        return res.status(500).send({ message: 'Failed to update blog post', data: null });
    }
});
