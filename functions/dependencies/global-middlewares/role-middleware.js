...
module.exports = {
    roleMiddleware: async (req, res, next) => {
        try {
            ...
            const [rows] = await getPromiseConnection().query(query, [req.admin.adminId, req.admin.brandId, req.method, `/${req._parsedUrl.pathname.split('/')[1]}`])

            if (rows.length) {
                next()
            } else {
                await endPromiseConnection(getPromiseConnection())
                return res.status(412).send({ message: 'Sorry! You don’t have access', data: null })
            }
        }
        catch (err) {
            await endPromiseConnection(getPromiseConnection())
